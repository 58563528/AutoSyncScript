...
func (c *Container) Fresh() error {
	...
	case "ql":
		if c.Version == "2.8" {
			...
			var data = c.request("/api/envs?searchValue=JD_COOKIE")
			a := AutoGenerated{}
			...
			c.request("/api/envs", POST, `{"name":"JD_COOKIE","value":"`+newValue+`"}`)
			return nil
		} else {
			var data = c.request("/api/cookies")
			type AutoGenerated struct {
				Code int `json:"code"`
				Data []struct {
					Value     string  `json:"value"`
			...
			if len(ids) > 0 {
				data = c.request("/api/cookies", DELETE, fmt.Sprintf(`[%s]`, strings.Join(ids, ",")))
			}
			newValue := []string{}
			cks := GetJdCookies()
			for _, ck := range cks {
			...
			c.request("/api/cookies", POST, fmt.Sprintf(`[%s]`, strings.Join(newValue, ",")))
			return nil
		}
	case "v4":
		config := ""
...
func (c *Container) request(ss ...string) []byte {
